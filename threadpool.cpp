...
struct threadpool_t{
   ...
   pthread_t *client_threads;                      /*存放客户端对象*/
   int client_num;
...
threadpool_create(int min_thr_num, int max_thr_num, int queue_max_size)
{
   ...
   do
   {
      ...
      {
        cout<<"malloc threadpool false;"<<endl;
	break;   
      }
      /*信息初始化*/
      ...
      pool->client_threads = new pthread_t[max_thr_num];
      pool->threads = new pthread_t[max_thr_num];
      if (pool->threads == NULL)
      {
        cout<<"malloc threads false;"<<endl;
	 break;
      }
    ...
    //   (threadpool_task_t *)malloc(sizeof(threadpool_task_t)*queue_max_size);
      ...
      {
         cout<<"malloc task queue false;"<<endl;
	 break;
      }
      ...
      pthread_rwlock_init(&(pool->wlock),NULL)!=0)
      {
         cout<<"init lock or cond false;"<<endl;
         break;
      }
...
threadpool_free(threadpool_t *pool)
{
   ...
   if (pool->threads)
   {
      ...
      pthread_rwlock_wrlock(&(pool->wlock));
      pthread_rwlock_destroy(&(pool->wlock));
   }
   free(pool);
   pool = NULL;
...
admin_thread(void *threadpool)
{
   ...
   //    cout<<"admin busy live "<< busy_thr_num<<" "<< live_thr_num<<endl;
...
threadpool_thread(void *threadpool)
{
  ...
  while (true)
  {
   ...
   pthread_rwlock_wrlock(&(pool->wlock));
    ...
    pthread_rwlock_unlock(&(pool->wlock));
   ...
   //cout<<(long*)task.arg<<endl;

    
    //pthread_join(pool->clients[pool->client_num++].thread,NULL);

                              //执行任务
   pthread_t t;
   pthread_create(&t, NULL, *task.function, task.arg);
   while(0);(*(task.function))(task.arg);
    //任务结束处理
...
//     threadpool_add_task(thp, printHello, (void*)&("abc"));
// }